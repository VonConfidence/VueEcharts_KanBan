<template>
  <div class="home-page">
    <nav-header>
      <template slot="average_msg">
        <data-item :data="item" v-for="(item,index) in items" :key="item.title" :isActiveItem="item.title == activeItemTitle" @changeActiveItem="changeActiveItem"></data-item>
      </template>

      <template slot="current_msg">
        讨论组
      </template>

      <template slot="chatmsg">
        聊天室
      </template>
